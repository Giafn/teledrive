...
import { clearPreviews, getPreviewBytes, setPreviewBytes } from '../lib/preview-cache';
import { enqueueUpload } from '../lib/upload-queue';
import { BrandMark, Icon } from './icon';
import { FileExplorer, fromFolderItem, fromObjectItem } from './file-explorer';
import type { ExplorerLayout, ExplorerSort } from './explorer-toolbar';
import type { DownloadAction, DownloadItem, SelectedEntry } from './types';
import { applyRoute, currentRoute, navigateRoute, type AppRoute } from '../lib/route-sync';
...
export function AppPage({ initialRoute }: { initialRoute?: AppRoute } = {}) {
  const [routeOverride] = useState<AppRoute | undefined>(() => initialRoute ?? currentRoute());
  ...
  const [view, setView] = useState<View>(routeOverride && routeOverride.view !== 'drive' ? routeOverride.view : 'drive');
  const [pendingFolderId, setPendingFolderId] = useState<string | undefined>(() =>
    routeOverride?.view === 'drive' ? routeOverride.folderId : undefined,
  );
  const [loading, setLoading] = useState(false);
  const [loadError, setLoadError] = useState('');
  const [query, setQuery] = useState('');
  const [layout, setLayout] = useState<ExplorerLayout>('grid');
  const [sort, setSort] = useState<ExplorerSort>('Terakhir diubah');
  ...
  async function loadFolder(id: string, nextCrumbs = crumbs) {
    ...
    try {
      ...
      const rootId = workspace?.rootFolder.id ?? nextCrumbs[0]?.id;
      if (rootId && id !== rootId) applyRoute({ view: 'drive', folderId: id });
      else applyRoute({ view: 'drive' });
    } catch (error) {
  ...
  async function enterApp(authUser: { displayName: string; username: string }) {
    ...
    try {
      ...
      setWorkspace(result);
      const deepId = pendingFolderId;
      setPendingFolderId(undefined);
      if (routeOverride && routeOverride.view !== 'drive') {
        if (routeOverride.view === 'settings') setView('settings');
        else await loadSpecial(routeOverride.view);
      } else if (deepId && deepId !== result.rootFolder.id) {
        try {
          const deep = await api.listFolderChildren(deepId, { limit: 100 });
          setCrumbs([
            { id: result.rootFolder.id, name: result.rootFolder.name },
            { id: deep.folder.id, name: deep.folder.name },
          ]);
          setFolder(deep);
          applyRoute({ view: 'drive', folderId: deepId });
        } catch {
  ...
  // Sinkronisasi tombol back/forward browser dengan view + folder.
  useEffect(() => {
    if (!user) return;
    const onPopState = () => {
      const route = currentRoute();
      if (route.view === 'recent' || route.view === 'trash') {
        void loadSpecial(route.view);
        return;
      }
      setView(route.view);
      if (route.view !== 'drive') return;
      const target = route.folderId ?? workspace?.rootFolder.id;
  ...
  async function loadSpecial(nextView: 'recent' | 'trash', append = false) {
    setView(nextView);
    if (!append) navigateRoute({ view: nextView });
    ...
    <main className={styles.app}>
      <aside className={styles.sidebar}>
        ...
        <div className={styles.sideBottom}>
          ...
          <button className={styles.settingsLink} onClick={() => {
            setView('settings');
      ...
      <section className={styles.content}>
        ...
        {view === 'settings' ? (
          ...
          <FileExplorer
            ...
            topActions={
              view === 'trash' ? (
                <button className={styles.secondaryButton} onClick={purgeTrash} disabled={specialLoading || !trash.length}>
                  Hapus semua
                </button>
              ) : undefined
            }
            items={(view === 'recent' ? recent : trash).map(fromObjectItem)}
            loading={specialLoading}
            loadError={loadError}
            onRetryLoad={() => loadSpecial(view as 'recent' | 'trash')}
            layout={layout}
            onLayoutChange={setLayout}
            sort={sort}
            onSortChange={setSort}
            query={query}
            menu={{ kind: view as 'recent' | 'trash' }}
            menuId={menuId}
            setMenuId={setMenuId}
            onMutate={mutate}
            ...
            dateField={view === 'trash' ? 'deleted' : 'updated'}
            loadMore={{
              cursor: specialCursor,
              loading: specialMoreLoading,
              onLoad: () => loadSpecial(view as 'recent' | 'trash', true),
            }}
            emptyContext={view as 'recent' | 'trash'}
          />
        ) : (
          <FileExplorer
            title={folder?.folder.name ?? 'Drive saya'}
            eyebrow={
              <div className={styles.eyebrow}>
            ...
            items={(folder?.items ?? []).map(fromFolderItem)}
            loading={loading}
            loadError={loadError}
            onRetryLoad={() => workspace && loadFolder(folder?.folder.id ?? workspace.rootFolder.id)}
            layout={layout}
            onLayoutChange={setLayout}
            sort={sort}
            onSortChange={setSort}
            ...
            setMenuId={setMenuId}
            onMutate={mutate}
            onOpenFolder={(item) => loadFolder(item.id, [...crumbs, { id: item.id, name: item.name }])}
            onPreviewFile={setPreview}
            onDownload={startDownload}
            downloads={downloads}
            selected={selected}
...
function PreviewModal({
  ...
  useEffect(() => {
    ...
    if (supported) {
      // Video yang gagal streaming: fallback unduh-penuh lalu putar lokal.
      if (useStream && streamFailed) {
        ...
        controller
          ...
          .then(({ bytes, mime }) => {
            ownUrl = URL.createObjectURL(new Blob([bytes as BlobPart], { type: mime }));
            urlRef.current = { revoke: () => undefined };
      ...
      if (cachedBytes) {
        ...
        controller
          .loadPreviewBytes(item.id, abort.signal)
          .then(({ bytes, mime }) => {
            if (isImage) setPreviewBytes(item.id, bytes, mime);
  ...
  }, [item.id, supported, useStream, streamFailed, isImage, item.thumbnail]);
  return (
    <div className={styles.previewBackdrop} role="presentation" onClick={onClose}>
      ...
      >
        ...
        <div className={styles.previewBody} onClick={onClose}>
          {!supported ? (
            ...
            />
          ) : useStream && streamFailed ? (
            <div className={styles.previewLoading} aria-live="polite">
              <span className={styles.spinner} />
              <b>Streaming tidak didukung browser ini…</b>
              <small>Mengunduh video untuk diputar lokal.</small>
