...
const worktree=()=>{const dir=fs.mkdtempSync(path.join(os.tmpdir(),'tl-b1-'));fs.rmSync(dir,{recursive:true,force:true});const result=spawnSync('git',['worktree','add','--detach',dir,'tl-supply-chain-b1-v4'],{cwd:repoRoot,encoding:'utf8'});if(result.status!==0)throw new Error(result.stderr);return path.join(dir,'packages/gramjs-browser');};
...
test('B1 v4 anchor scopes trust to protected paths',()=>{
